...
void deleteNode(Node * &head, Node *NodeDel)
{
    if(head == NodeDel)
    {
        head = head->next;
    ...
    Node *prev = head;
    while(prev->next != nullptr && prev->next != NodeDel)
    {
        prev = prev->next;
    }
    if (prev->next == NodeDel)
    {
...
int main()
{
 ...
 deleteDup(head);
 cout<<"\nAfter Deleting duplicates ",,endl;
 display(head);
